...
exports.Login = async (req, res) => {
  try {
    ...
    //1. Find if any account with that email exists in Database
    ...
    //2. thorw error if account is not activated
    if (!user.active) {
      return res.status(400).json({
        error: true,
...
exports.Activate = async (req, res) => {
  try {
    ...
    const user = await User.findOne({
      email: email,
      emailToken: code,
      emailTokenExpires: { $gt: Date.now() }, // check if code is expired
    });
